...
float4 MainPS(VertexShaderOutput input) : SV_Target
{
    ...
    float waveCenter = 0.5 + sin(input.TextureCoordinates.x * 28.0 - iTime * 4.0) * 0.12;
    float acrossLine = abs(input.TextureCoordinates.y - waveCenter);
    float pulse = exp2(-11.0 * alongLine) * exp2(-11.0 * acrossLine) * isTraveling;
    ...
    float brightness = (0.18 + pulse * 1.35) * (0.4 + 1.35 * centerGlow);

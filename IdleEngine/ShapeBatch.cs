...
namespace Particles
{
	...
	public static class ShapeBatch
	{
        ...
        // The textured batches of lines and polygons
        private static List<VertexPositionTexture> texturedPolygons = new List<VertexPositionTexture>(); // Added by Joseph Staria
        ...
        /// Internal initialize function that is called any time
        /// a batch is started.  Only performs initialization 
        /// once per program. Added by Joseph Staria
        /// </summary>
        /// <param name="device">The GraphicsDevice used for drawing</param>
        private static void InitializeTextured(GraphicsDevice device, Texture2D texture, RenderTarget2D renderTarget)
        {
            // Anything to do?
            if (initialized)
                return;
            ...
            // Save the device so we can draw whenever we need to
            ShapeBatch.device = device;
			ShapeBatch.device.SetRenderTarget(renderTarget);
		...
		/// <summary>
		/// Begins a batch. Shapes can only be drawn during a batch. Joseph Staria addition
		/// </summary>
		/// <param name="device"></param>
		public static void BeginTextured(GraphicsDevice device, Texture2D texture, RenderTarget2D renderTarget)
		{
			if (batchActive)
                throw new InvalidOperationException("Cannot call Begin() twice before calling End()");

            // Initialize the draw helper if necessary
            InitializeTextured(device, texture, renderTarget);
        ...
        /// <param name="height">The height of the box</param>
        /// <param name="color">The color of the box</param>
        public static void BoxTextured(float x, float y, float width, float height)
        {
            if (!batchActive)
                throw new InvalidOperationException("Box() must be called between Begin() and End()");
            ...
            Vector2 topLeftUV = new Vector2(0, 0); // 0
            Vector2 topRightUV = new Vector2(1, 0); // 1
            Vector2 bottomLeftUV = new Vector2(0, 1); // 2
            Vector2 bottomRightUV = new Vector2(1, 1); // 3
            

            // Create the polygons
            BatchPolygon(topLeft, bottomRight, bottomLeft, topLeftUV, bottomRightUV, bottomLeftUV);
            BatchPolygon(topLeft, topRight, bottomRight, topLeftUV, topRightUV, bottomRightUV);
        ...
        /// <summary>
        /// Draws a solid (filled-in) box. Added by Joseph Staria
        /// </summary>
        /// <param name="rect">The rectangle specifying the box's position and size</param>
        /// <param name="color">The color of the outline</param>
        public static void Box(Rectangle rect)
        {
            ...
            BoxTextured(rect.X, rect.Y, rect.Width, rect.Height);
        }
        ...
        /// <summary>
        /// Adds a single triangle to the polygon batch.  Potentially causes a Flush()
        /// if there are already enough primitives in the polygon batch. Added by Joseph Staria
        /// </summary>
        /// <param name="p0">First vertex position</param>
        /// <param name="p1">Second vertex position</param>
        /// <param name="p2">Third vertex position</param>
        ...
        /// <param name="color1">The color of the second vertex</param>
        /// <param name="color2">The color of the third vertex</param>
        private static void BatchPolygon(Vector2 p0, Vector2 p1, Vector2 p2, Vector2 uv0, Vector2 uv1, Vector2 uv2)
        {
            // Any need to flush?
            if (texturedPolygons.Count / 3 >= PrimitivesPerBatch)
            {
                // Need to flush both sets (lines and polygons) since their
                // depths may be interleaved at this point
                FlushShapes();
            ...
            // Add the vertices to the polygon list
            texturedPolygons.Add(new VertexPositionTexture(PixelsToNDCs(p0, currentDepth), uv0));
            texturedPolygons.Add(new VertexPositionTexture(PixelsToNDCs(p1, currentDepth), uv1));
            texturedPolygons.Add(new VertexPositionTexture(PixelsToNDCs(p2, currentDepth), uv2));
        }
        ...
        private static void FlushShapes()
		{
            ...
            if (texturedPolygons.Count > 0)
            {
                device.DrawUserPrimitives(PrimitiveType.TriangleList, texturedPolygons.ToArray(), 0, texturedPolygons.Count / 3);
                texturedPolygons.Clear();
            }
